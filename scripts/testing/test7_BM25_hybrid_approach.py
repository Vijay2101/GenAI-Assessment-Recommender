...
# Build BM25 corpus for fallback scoring
def tokenize(text: str):
    return re.findall(r"\b\w+\b", text.lower())

bm25_corpus = []
for item in catalog:
    text = f"{item.get('assessment_name', '')} {item.get('description', '')}"
    bm25_corpus.append(tokenize(text))

bm25 = BM25Okapi(bm25_corpus)


def bm25_retrieve(query: str, top_k: int = 40):
...
def hybrid_retrieve(query: str, query_vec):
    # Dense retrieval
    _, dense_indices = index.search(query_vec.reshape(1, -1), TOP_K_RETRIEVAL)
    dense_results = [catalog[i] for i in dense_indices[0]]

    # Sparse retrieval
    sparse_results = bm25_retrieve(query, TOP_K_RETRIEVAL)

    # Merge + deduplicate
    merged = {}
    for item in dense_results + sparse_results:
        merged[item["assessment_url"]] = item
...
def run_test(query: str):
    ...
    query_vec = embed_query(query)
    candidates = hybrid_retrieve(query, query_vec)
    print(f"\nRetrieved candidates (hybrid): {len(candidates)}")
